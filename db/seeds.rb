...
ArrayArrayArray = Challenge.create(
  ...
  \n\nFor example: if (array) == ["a", 3] \nyou should return [["a", 3], ["a", 3], ["a", 3]].',
  language: 'Ruby',
  tests: {
    ['a', 3] => [['a', 3], ['a', 3], ['a', 3]],
...
PrintedErrors = Challenge.create(
  ...
  which, are named with letters from a to m.
  \nThe colors used by the printer are recorded in a control string.
  \nFor example a "good" control string would be aaabbbbhaijjjm meaning that the printer used color a three times, color b four times, color h once, then
  once again color a...
  \nSometimes there are problems: lack of colors, technical malfunctions, and a "bad" control string is produced for example:
